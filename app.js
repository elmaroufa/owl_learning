...
class Task extends Component{
  static template = xml `
  <li  t-attf-style="background-color:#{state.color}" class="d-flex align-items-center 
  justify-content-between border p-3 mb-2 rounded">
  <div t-if="state.isEditing" class="d-flex align-items-center flex-grow-1 me-2">
    <input type="text" class="form-control m-2"  t-model="state.name"/>
    <input type="color" style="width:60px" class="form-control-lg form-control-color border-0 bg-white" id="color" 
  ...
  <div t-if="!state.isEditing" class="form-check form-switch fs-5">
    ...
    <div>
      <button t-if="!state.isEditing" class="btn btn-primary m-2" t-on-click="editTask"><i class="bi bi-pencil"></i></button>
      <button t-if="state.isEditing" class="btn btn-primary m-2" t-on-click="saveTask"><i class="bi bi-check"></i></button>
